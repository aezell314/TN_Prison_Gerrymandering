...
sldl_pop <- read_csv('../Data/tn_cvap_2023_sldl/tn_cvap_2023_sldl.csv') |>
  select(SLDL, CVAP_TOT23, ALL_TOT23) |>
...
sldu_pop <- read_csv('../Data/tn_cvap_2023_sldu/tn_cvap_2023_sldu.csv') |>
  select(SLDU, CVAP_TOT23, ALL_TOT23) |>
  rename(cvap_total=CVAP_TOT23,
         total_pop = ALL_TOT23)
...
TN_voter_data <- read_csv('../Data/TN_l2_2024_gen_stats_2020block/TN_l2_2024_gen_stats_2020block.csv') |>
  ...
  select(-starts_with(c('voted', 'pct_voted', 'reg_self_afam')))  # drop specific voter turnout percentages except for overall turnout
...
sldl_demogr <- sldl_demogr |>
  ...
  select(-(starts_with('pct_age'))) |>
  relocate(c(prison_pop, total_pop, cvap_total, pct_incarc, vote_strength, prisons_present), .after = SLDL)
...
sldu_demogr <- sldu_demogr |>
  mutate(pct_incarc = prison_pop / cvap_total * 100,
         ...
         pct_65_plus = pct_age_65_74 + pct_age_75_84) |>
  select(-(starts_with('pct_age'))) |>
  relocate(c(prison_pop, total_pop, cvap_total, pct_incarc, vote_strength, prisons_present), .after = SLDU)
```
...
sldl_demogr <- sldl_demogr |>
  ...
  relocate(pop_weight, pop_adj, cvap_adj, .after = SLDL)
```



Would any of TN's legislative districts be legally too small if inmates were redistributed?

```{r}
# Calculating percent difference between the largest and smallest districts after redistributing the prison population
largest_district <- sldl_demogr |> slice_max(pop_adj) |> select(pop_adj) |> pull()
smallest_district <- sldl_demogr |> slice_min(pop_adj) |> select(pop_adj) |> pull()

((largest_district - smallest_district)/smallest_district)*100
```
After redistributing the prison population, the largest district is 19.86% larger than the smallest district, which is greater than the generally accepted 10% threshold.
...
sldu_demogr <- sldu_demogr |>
  mutate(pop_weight = (total_pop - prison_pop)/tot_nonprison_pop,
         ...
         # Create a new column for vote strength, which is voting population minus prison population divided by adjusted population
         vote_strength = (cvap_total - prison_pop)/cvap_adj) |>
  relocate(pop_weight, pop_adj, cvap_adj, .after = SLDU)
```


```{r}
# Calculating percent difference between the largest and smallest districts after redistributing the prison population
largest_district <- sldu_demogr |> slice_max(pop_adj) |> select(pop_adj) |> pull()
smallest_district <- sldu_demogr |> slice_min(pop_adj) |> select(pop_adj) |> pull()

((largest_district - smallest_district)/smallest_district)*100
```
After redistributing the prison population, the largest district is 15% larger than the smallest district, which is greater than the generally accepted 10% threshold.
...
```
The presence of a prison does not have a statistically significant effect on voting strength.
...
sldl_dem_prisons = lm(pct_party_democratic ~ prisons_present, data = sldl_demogr)
...
```
There is not a statistically significant relationship between the presence of a prison and the percent of the voting age population that is 65 or older.  
...
sldl_turnout_prisons = lm(pct_party_democratic ~ prisons_present, data = sldl_demogr)
...
```{r}
# creating a linear regression model with x = prisons_present, and y = percent of voting age population registered as Democrat
sldu_dem_prisons = lm(pct_party_democratic ~ prisons_present, data = sldu_demogr)
...
Do districts with prisons have significantly different demographic makeup than districts without prisons?
```{r}
# creating a linear regression model with x = prisons_present, and y = percent of voting age population that is male
...
```{r}
# creating a linear regression model with x = prisons_present, and y = voter turnout
sldu_turnout_prisons = lm(pct_party_democratic ~ prisons_present, data = sldu_demogr)
