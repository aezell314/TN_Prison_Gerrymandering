...
```{r}
prisons_sldl <- prisons_sldl |>
  # add an indicator column for whether or not the district contains a prison
  mutate(has_prison = !is.na(name))

prisons_sldu <- prisons_sldu |>
  # add an indicator column for whether or not the district contains a prison
  mutate(has_prison = !is.na(name))

#merge indicator column into district boundary dataframes
TN_house_bounds <- full_join(
  TN_house_bounds,
  (prisons_sldl |> st_drop_geometry() |> group_by(DISTRICT) |> summarize(has_prison = max(has_prison))),
  by = 'DISTRICT')

TN_senate_bounds <- full_join(
  TN_senate_bounds,
  (prisons_sldu |> st_drop_geometry() |> group_by(DISTRICT) |> summarize(has_prison = max(has_prison))),
  by = 'DISTRICT')
```
...
sldl_demogr <- sldl_demogr |>
  mutate(pop_weight = (total_pop - prison_pop)/tot_nonprison_pop,
         ...
         # Create new columns for vote strength, which is total population divided by eligible voters (non-incarcerated) 
         vote_strength_adj = cvap_adj/(cvap_total - prison_pop),
         vote_strength = cvap_total/(cvap_total - prison_pop),
         # Create new column for percent of population that is incarcerated (after adjustment)
         pct_incarc_adj = prison_pop_adj/total_pop*100) |>
  relocate(pop_adj, cvap_adj, pct_incarc_adj, .after = SLDL) |>
  # drop weights since they are not needed
  dplyr::select(-pop_weight)
```

Creating tables showing greatest prison population percent for slideshow:
```{r}
sldl_prison_pct <- sldl_demogr |>
  select(SLDL, total_pop, prison_pop, vote_strength) |>
  mutate(pct = prison_pop/total_pop*100) |>
  mutate(across(where(is.numeric), round, digits = 2)) |>
  mutate(total_pop=prettyNum(total_pop, big.mark = ","),
         prison_pop=prettyNum(prison_pop, big.mark = ","),
         pct = paste0(pct,'%')) |>
  slice_max(pct, n=5) |>
  rename(District=SLDL, 'Total population'=total_pop, 'Incarcerated population'=prison_pop, 'Prison population as percent of total population'=pct, 'Vote strength'=vote_strength)

sldu_prison_pct <- sldu_demogr |>
  select(SLDU, total_pop, prison_pop, vote_strength) |>
  mutate(pct = prison_pop/total_pop*100) |>
  mutate(across(where(is.numeric), round, digits = 2)) |>
  mutate(total_pop=prettyNum(total_pop, big.mark = ","),
         prison_pop=prettyNum(prison_pop, big.mark = ","),
         pct = paste0(pct,'%')) |>
  slice_max(pct, n=5) |>
  rename(District=SLDU, 'Total population'=total_pop, 'Incarcerated population'=prison_pop, 'Prison population as percent of total population'=pct, 'Vote strength'=vote_strength)
```
...
```{r}
sldl_pop_dev <- sldl_demogr |>
  select(SLDL, pct_pop_dev, pct_pop_dev_adj) |>
  arrange(desc(abs(pct_pop_dev_adj))) |>
  mutate(across(where(is.numeric), round, digits = 2),
         across(pct_pop_dev:pct_pop_dev_adj, paste0, '%')) |>
  rename(District=SLDL, 'Population Deviation'=pct_pop_dev, 'Population Deviation (Adjusted)'=pct_pop_dev_adj) |>
  head(5)

sldu_pop_dev <- sldu_demogr |>
  select(SLDU, pct_pop_dev, pct_pop_dev_adj) |>
  arrange(desc(abs(pct_pop_dev_adj))) |>
  mutate(across(where(is.numeric), round, digits = 2),
         across(pct_pop_dev:pct_pop_dev_adj, paste0, '%')) |>
  rename(District=SLDU, 'Population Deviation'=pct_pop_dev, 'Population Deviation (Adjusted)'=pct_pop_dev_adj) |>
  head(5)
```
...
sldu_demogr <- sldu_demogr |>
  mutate(pop_weight = (total_pop - prison_pop)/tot_nonprison_pop,
         ...
         prison_pop_adj = tot_prison_pop*pop_weight,
         cvap_adj = cvap_total - prison_pop + (pop_weight * tot_prison_pop),
         # Create new columns for vote strength, which is total voting age population divided by eligible voters (non-incarcerated) 
         vote_strength_adj = cvap_adj/(cvap_total - prison_pop),
         vote_strength = cvap_total/(cvap_total - prison_pop),
         # Create new column for percent of population that is incarcerated (after adjustment)
         pct_incarc_adj = prison_pop_adj/total_pop*100) |>
  relocate(pop_adj, cvap_adj, pct_incarc_adj, .after = SLDU) |>
  # drop weights since they are not needed
  dplyr::select(-pop_weight)
```
...
lm_prisons_present = lm(vote_strength ~ num_prisons, data = sldl_demogr)
...
```{r}
# creating a linear regression model with x = presence of 1 or more prisons, and y = vote strength
sldu_prisons_present = lm(vote_strength ~ num_prisons, data = sldu_demogr)
...
```{r}
saveRDS(prisons_sldu, file = "../TN_Prison_ShinyApp/Data/prisons_sldu.rds")
saveRDS(prisons_sldl, file = "../TN_Prison_ShinyApp/Data/prisons_sldl.rds")
saveRDS(sldu_demogr, file = "../TN_Prison_ShinyApp/Data/sldu_demogr.rds")
saveRDS(sldl_demogr, file = "../TN_Prison_ShinyApp/Data/sldl_demogr.rds")
saveRDS(TN_house_bounds, file = "../TN_Prison_ShinyApp/Data/TN_house_bounds.rds")
saveRDS(TN_senate_bounds, file = "../TN_Prison_ShinyApp/Data/TN_senate_bounds.rds")
saveRDS(prison_info, file = "../TN_Prison_ShinyApp/Data/prison_info.rds")
readr::write_csv(sldl_prison_pct, "../TN_Prison_ShinyApp/Data/sldl_prison_pct.csv")
readr::write_csv(sldu_prison_pct, "../TN_Prison_ShinyApp/Data/sldu_prison_pct.csv")
readr::write_csv(sldl_pop_dev, "../TN_Prison_ShinyApp/Data/sldl_pop_dev.csv")
readr::write_csv(sldu_pop_dev, "../TN_Prison_ShinyApp/Data/sldu_pop_dev.csv")
```



```{r}
current_pop <- sldl_demogr |>
      filter(SLDL==16) 
```
