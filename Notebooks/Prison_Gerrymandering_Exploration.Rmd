...
```{r}
# Merge demographic data into district boundary data
district_bounds_statewide <- full_join(TN_house_bounds, sldl_demogr |> rename(DISTRICT=SLDL), by='DISTRICT')
```
...
pal <- colorNumeric(
  palette = "plasma", 
  domain = district_bounds_statewide$pct_party_democratic)
...
addPolygons(
    ...
    weight = 1,
    fillOpacity = 0.5,
    fillColor = ~pal(pct_party_democratic), 
    highlightOptions = highlightOptions(
      color = "red", weight = 3, bringToFront = TRUE
    ),
    label = ~LONGNAME
  ...
  addLegend(
    position = "bottomleft",
    pal = pal,
    values = ~pct_party_democratic,
    title = "Percent Democratic",
