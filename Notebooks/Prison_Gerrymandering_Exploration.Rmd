...
```{r}
# creating a linear regression model with x = prisons_present, and y = percent of voting age population registered as Democrat
sldl_dem_prisons = lm(pct_party_democratic ~ pct_pop_dev_adj, data = sldl_demogr)
summary(sldl_dem_prisons)
```
...
```{r}
# creating a linear regression model with x = prisons_present, and y = percent of voting age population registered as Democrat
sldu_dem_prisons = lm(pct_party_democratic ~ pct_pop_dev_adj, data = sldu_demogr)
summary(sldu_dem_prisons)
```

There is not a statistically significant relationship between population deviation and the partisan makeup of state house districts; for state senate districts, there is a statistically significant relationship - each additional percentage of population deviation results in about a 1% decrease in the percentage of voters who are registered as Democrats, meaning the larger state senate districts are more likely to be heavily Republican.  
